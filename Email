...
function getDecisionEmailPreview(batchId, decisions, batchNote) {
  ...
  var subject = '[' + settings.schoolName + '] Academic Exception Review: ' + reviewData.request.batchId;
  var htmlBody = buildDecisionEmailHtml_(reviewData, settings);
  var plainBody = buildDecisionEmailPlainText_(reviewData, settings);

  return {
    to: reviewData.request.teacherEmail,
    cc: reviewData.principalCc.join(', '),
    subject: subject,
    htmlBody: htmlBody,
    plainBody: plainBody,
...
function sendDecisionEmail(batchId, decisions, batchNote) {
  ...
  var preview = getDecisionEmailPreview(batchId, decisions, batchNote);
  var reviewData = buildBatchReviewData_(batchId, decisions, batchNote);
  var settings = getAppSettings();
